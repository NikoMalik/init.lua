...
vim.keymap.set('n', '<leader>cc', '<cmd>BufferLinePickClose<cr>', { desc = 'Pick buffer to close' })
...
local function interactive_replace()
	...
	vim.ui.input({ prompt = 'Yo! Enter what you want to replace: ' }, function(search)
		...
		vim.ui.input({ prompt = 'Enter new string (leave empty to delete): ' }, function(replace)
			...
			vim.notify('Performing replace in ' .. scope .. ': "' .. search .. '" → "' .. replace .. '"',
				vim.log.levels.INFO)
...
vim.api.nvim_create_user_command('ReplaceString', interactive_replace, {
	desc = 'Interactive replace' })
...
require('lazy').setup({
	...
	},
	{
		...
		config = function(_, opts)
			...
			local function conflict_colors()
				vim.api.nvim_set_hl(0, 'GitConflictCurrent', { bg = '#40151a', bold = true })
				vim.api.nvim_set_hl(0, 'GitConflictCurrentLabel', { bg = '#6e2530', bold = true })
				vim.api.nvim_set_hl(0, 'GitConflictIncoming', { bg = '#14351a', bold = true })
				vim.api.nvim_set_hl(0, 'GitConflictIncomingLabel', { bg = '#255e30', bold = true })
				vim.api.nvim_set_hl(0, 'GitConflictAncestor', { bg = '#2a2a30', bold = true })
				vim.api.nvim_set_hl(0, 'GitConflictAncestorLabel', { bg = '#45454f', bold = true })
			end

			conflict_colors()
			vim.api.nvim_create_autocmd('ColorScheme', {
				group = vim.api.nvim_create_augroup('GitConflictColors', { clear = true }),
				callback = conflict_colors,
			})
		end,
	},
	-- {
	...
	},
	{
		...
		config = function()
			...
			-- vim.api.nvim_set_hl(0, '@lsp.type.operator', { fg = blue })
			-- vim.api.nvim_set_hl(0, '@lsp.typemod.operator', { fg = blue })
			--
			-- -- For C/C++ specific
			-- vim.api.nvim_set_hl(0, '@type.qualifier.c', { fg = blue })
			-- vim.api.nvim_set_hl(0, '@type.qualifier.cpp', { fg = blue })
		end,
	},
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		dependencies = {
			...
			{ -- If encountering errors, see telescope-fzf-native README for installation instructions
				...
				cond = function()
					return vim.fn.executable 'cmake -S. -Bbuild -DCMAKE_BUILD_TYPE=Release && cmake --build build --config Release --target install' ==
						1
				end,
			},
